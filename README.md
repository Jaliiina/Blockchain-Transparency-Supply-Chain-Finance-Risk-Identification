...
系统采用 Geth 的 --dev 模式 启动轻量级本地链，特点包括：
- 无需挖矿等待，区块即时打包；
- 自动创建预置账户；
- Chain ID = 1337（适用于测试及脚本化部署）；
- 支持 Remix、Web3.py 等工具实时连接。
示例启动命令：
```
geth --dev --rpc --rpcaddr 127.0.0.1 --rpcport 8545 --rpcapi "eth,net,web3"
```
所有链上交互（部署合约、调用函数、读取区块信息）均基于此节点完成。

2. 智能合约开发与部署流程
智能合约在 Solidity 0.8.21 + Remix IDE 中开发、编译与部署。
本项目采用的 Hello.sol 合约为区块链交互的最小示例，用于验证：
- 编译是否正常
- 部署是否成功
- Web3 是否能从本地链读取 ABI
- 端是否能发起合约调用
...
部署成功后生成链上地址（例如）：
- 0xE46E3CF72446FC62e00C7d4d

ABI 通过 Remix artifacts 自动导出，存储于：
'''
contracts/hello_abi.json
'''

3. 区块链客户端集成（Web3.py）
系统使用 Web3.py 建立后端与本地链的连接：
```
w3 = Web3(Web3.HTTPProvider("http://127.0.0.1:8545"))
contract = w3.eth.contract(address=HELLO_CONTRACT_ADDRESS, abi=hello_abi)
```
通过 /api/hello 接口，前端可实时调用链上函数，例如：
```
contract.functions.sayHello().call()
```
返回值将在前端立即展示，用于验证链上应用逻辑。
